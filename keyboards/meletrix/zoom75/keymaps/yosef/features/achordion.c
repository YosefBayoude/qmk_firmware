...
enum {
  ...
  STATE_HOLDING,
  // Settled as tap but the tap key is held down (released on physical key release).
  STATE_TAP_HOLDING,
...
static void process_eager_mods_action(void) {
  ...
  action.code = ACTION_MODS_TAP_KEY(
      eager_mods,
      QK_MOD_TAP_GET_TAP_KEYCODE(tap_hold_keycode)
  );
...
// Sends tap press (but not release) and settles as tap-holding.
// The tap key release is deferred until the physical tap-hold key is released.
static void settle_as_tap_hold(void) {
  if (eager_mods) {
    tap_hold_record.event.pressed = false;
    action_t action;
    action.code = ACTION_MODS(eager_mods);
  ...
  dprintln("Achordion: Plumbing tap hold press.");
  tap_hold_record.event.pressed = true;
  tap_hold_record.tap.count = 1;
  tap_hold_record.tap.interrupted = true;
  recursively_process_record(&tap_hold_record, STATE_TAP_HOLDING);
  send_keyboard_report();
}
...
bool process_achordion(uint16_t keycode, keyrecord_t *record) {
  ...
  if (achordion_state == STATE_RELEASED) {
    if (is_tap_hold && record->tap.count == 0 && record->event.pressed &&
      ...
      if (timeout > 0) {
        ...
        dprintf(
            "Achordion: Key 0x%04X pressed.%s\n",
            keycode,
  ...
  if (keycode == tap_hold_keycode && !record->event.pressed) {
    ...
    } else if (achordion_state == STATE_TAP_HOLDING) {
      dprintln("Achordion: Key released. Plumbing tap hold release.");
      tap_hold_record.event.pressed = false;
      tap_hold_record.tap.count = 1;
      recursively_process_record(&tap_hold_record, STATE_RELEASED);
  ...
  if (achordion_state == STATE_UNSETTLED && record->event.pressed) {
    ...
    if (!is_streak &&
      ...
      if (achordion_hold_on_tap(tap_hold_keycode, keycode)) {
        settle_as_tap_hold();
      } else {
        settle_as_tap();
      }
...
__attribute__((weak)) bool achordion_chord(
...
// By default, don't hold the tap key.
__attribute__((weak)) bool achordion_hold_on_tap(
    uint16_t tap_hold_keycode, uint16_t other_keycode) {
  return false;
}
